...
import deerangle.space.planets.Planet;
...
public class VenusAtmosphereRenderer extends AtmosphereRenderer {
    ...
    private float sunOrbitDegrees = 0;
    private float dayLength = 118 * 24000;
    private float sunRiseDirection = -90.0F; //-90 is east, 90 is west, 0 is south, 180 is north ?
    private List<Planet> orbitingPlanets;

    public VenusAtmosphereRenderer() {
        generateSky();
        generateSky2();
        generateStars();
    }
    ...
    protected void renderSky(int ticks, float partialTicks, MatrixStack matrixStackIn, ClientWorld world, Minecraft mc) {
        ...
        for (Planet planet : this.orbitingPlanets) {
            ...
            matrixStackIn.rotate(Vector3f.XP.rotationDegrees(planet.getSkyPosition(world.func_242415_f(partialTicks) * 360.0F)));
