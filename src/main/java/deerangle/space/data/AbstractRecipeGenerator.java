...
public class AbstractRecipeGenerator extends RecipeProvider {
    ...
    protected void addStoneCutterRecipe(Consumer<IFinishedRecipe> consumer, IItemProvider ingredient, IItemProvider result, int count, String recipeName) {
        SingleItemRecipeBuilder.stonecuttingRecipe(Ingredient.fromItems(ingredient), result, count).addCriterion("has_ingredient", hasItem(ingredient)).build(consumer, recipeName + "_from_stone_cutting");
    }
    ...
    protected void addSpaceBlastFurnaceRecipe(Consumer<IFinishedRecipe> consumer, Ingredient input, Item result, int duration, String recipeName) {
        ...
        consumer.accept(new BlastFurnaceResult(new ResourceLocation(loc.getNamespace(), loc.getPath() + recipeName + "_from_space_blast_furnace"), input, result, duration));
    }
    ...
    protected void addRefineryRecipe(Consumer<IFinishedRecipe> consumer, FluidStack input, FluidStack result, int duration) {
        ...
        consumer.accept(new RefineryResult(new ResourceLocation(loc.getNamespace(), loc.getPath() + "_from_refinery"), input, result, duration));
    }

    protected void addMachineRemoveNBTRecipe(Block block, Consumer<IFinishedRecipe> consumer) {
        String name = block.getRegistryName().getPath();
        ShapelessRecipeBuilder.shapelessRecipe(block).addIngredient(block).addCriterion("has_" + name, hasItem(block)).build(consumer, block.getRegistryName().getNamespace() + ":" + name + "_remove_nbt");
    }
    ...
    protected void registerIngotRecipes(Item itemIngot, Block itemBlock, Item itemNugget, Consumer<IFinishedRecipe> consumer, String name) {
        ShapelessRecipeBuilder.shapelessRecipe(itemIngot, 9).addIngredient(itemBlock).addCriterion("has_ingot", hasItem(itemIngot)).build(consumer, name + "_ingot_from_" + name + "_block");
        ShapelessRecipeBuilder.shapelessRecipe(itemBlock).addIngredient(itemIngot, 9).addCriterion("has_ingot", hasItem(itemIngot)).build(consumer);
        ShapelessRecipeBuilder.shapelessRecipe(itemIngot).addIngredient(itemNugget, 9).addCriterion("has_ingot", hasItem(itemIngot)).build(consumer, name + "_ingot_from_" + name + "_nugget");
        ShapelessRecipeBuilder.shapelessRecipe(itemNugget, 9).addIngredient(itemIngot).addCriterion("has_ingot", hasItem(itemIngot)).build(consumer);
    }
